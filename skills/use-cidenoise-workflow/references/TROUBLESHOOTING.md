...
| Existing output destination | Preserve the completed output and choose a new destination for another run. There is no overwrite flag. Existing gallery PNGs are also rejected. |
...
For visual gallery failures, inspect the failed model panels, embedded PNG metadata
and execution log. Choose a new output folder for a rerun. All successful model
results use the same raw-derived display ranges; visual smoothness alone does not
establish that faint structures are preserved.
