# Inference subset of UniFMIR; see THIRD_PARTY.md.
import math
import torch
...
class swinir(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=1, out_chans=1, embed_dim=180 // 2, depths=[6, 6, 6], num_heads=[6, 6, 6], window_size=8, mlp_ratio=2.0, qkv_bias=True, qk_scale=None, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, norm_layer=nn.LayerNorm, ape=False, patch_norm=True, use_checkpoint=False, upscale=2, img_range=1.0, num_feat=32):
        ...
        if upscale == 11:
            self.conv_before_upsample = nn.Sequential(nn.Conv2d(embed_dim, num_feat * 4, 3, 1, 1), nn.LeakyReLU(inplace=True))
            for i in range(3):
                self.uplayers.append(nn.Sequential(Upsample2(num_feat), nn.Conv2d(num_feat, num_feat * 4, 3, 1, 1), nn.BatchNorm2d(num_features=num_feat * 4), nn.ReLU(inplace=True)))
            self.uplayers.append(nn.Sequential(Upsample2(num_feat), nn.Conv2d(num_feat, num_feat, 3, 1, 1)))
        else:
