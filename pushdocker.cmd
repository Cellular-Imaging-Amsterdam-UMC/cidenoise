...
if /I not "%FULL_IMAGE%"=="cellularimagingcf/w_cidenoise" (
    echo ERROR: Refusing to publish to unexpected Docker repository: %FULL_IMAGE%
    echo Expected: cellularimagingcf/w_cidenoise
    exit /b 1
)
...
popd >nul
if "%DRY_RUN%"=="1" (
    echo Dry run completed for %FULL_IMAGE% version %VERSION%.
) else (
    echo Pushed %FULL_IMAGE% version %VERSION%.
)
endlocal & exit /b 0
