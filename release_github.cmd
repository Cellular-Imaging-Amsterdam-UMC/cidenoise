...
"%LOCALAPPDATA%\miniconda3\envs\cidenoise\python.exe" "%REPO_ROOT%\tools\sync_config_version.py" "%REPO_ROOT%\config.yaml" "%TAG%" --check >nul
...
git remote get-url origin | powershell -NoProfile -Command "$u=($input | Out-String).Trim(); $allowed=@('https://github.com/Cellular-Imaging-Amsterdam-UMC/cidenoise.git','git@github.com:Cellular-Imaging-Amsterdam-UMC/cidenoise.git'); if ($allowed -notcontains $u) { exit 1 }"
if errorlevel 1 (
    echo ERROR: origin does not point to Cellular-Imaging-Amsterdam-UMC/cidenoise.
    goto :fail
)
echo Origin repository verified: Cellular-Imaging-Amsterdam-UMC/cidenoise
...
if "%CONFIG_SYNC_NEEDED%"=="1" (
    ...
    call :run_or_echo "%LOCALAPPDATA%\miniconda3\envs\cidenoise\python.exe" "%REPO_ROOT%\tools\sync_config_version.py" "%REPO_ROOT%\config.yaml" "%TAG%"
    if errorlevel 1 goto :fail
